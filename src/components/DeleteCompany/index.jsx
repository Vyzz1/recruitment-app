...
import { deleteCompanyAll } from "../../action/deleteCompanyAll";
function DeleteCompany({ items }) {
  ...
  const handleFinish = ({ name, password }) => {
    ...
    setTimeout(() => {
      if (name === items.companyName && password === "admin") {
        dispatch(deleteCompanyAll(items.key));
    ...
    <>
      ...
      >
        <Form onFinish={handleFinish}>
          ...
          >
            <Input placeholder={` Hãy nhập đúng ${items?.companyName}`} />
          </Form.Item>
          <Form.Item
            label="Mật khẩu"
            name="password"
